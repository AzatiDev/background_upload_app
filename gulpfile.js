...
gulp.task('css', function() {
	return gulp.src(['./src/css/**/*.css'])
		...
		.pipe(concat('app.css'))
        .pipe(gulp.dest(develFolder + '/css/'));
});
...
gulp.task('watch', function() {
    // prepare project
    gulp.start('views', 'source-js', 'libs-js', 'images', 'css', 'root-page', 'fonts');
...
gulp.task('build_css', function() {
    return gulp.src(['./src/css/**/*.css'])
        .pipe(cssmin())
		.pipe(concat('app.css'))
        .pipe(gulp.dest(buildDestFolder + '/css/'));
});
...
gulp.task('build_files', function() {
    return gulp.src(['./src/index.html', './src/config.xml', './src/icon.png'], { 'base': './src/' })
		.pipe(gulp.dest(buildDestFolder));
});
...
gulp.task('compress_build', ['build_css', 'build_fonts', 'build_pages', 'build_libs', 'build_js', 'build_img', 'build_files', 'build_res'], function() {
    return gulp.src(buildDestFolder + '/**/*')
        .pipe(zip('background_upload.zip'))
        .pipe(gulp.dest('./'));
});

gulp.task('build', ['build_css', 'build_fonts', 'build_pages', 'build_libs', 'build_js', 'build_img', 'build_files', 'build_res', 'compress_build']);
